...
void Window::unmap() {
  ...
  if (w.map_pending_ || is_mapped()) {
    ...
    w.unmapped_ = true;
    w.wm_mapped_ = false;
    // added by cd1f 11-may-95
    // for some reason this was occasionaly left on
    // and menus would not map thier windows correctly
...
unsigned long WindowVisual::x_or(const Style& s) const {
    unsigned long p;
    String custom;
...
static inline double mm_to_points(double mm_val) {
    return (72.0 / 25.4) * mm_val;
}
