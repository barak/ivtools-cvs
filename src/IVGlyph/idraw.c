...
Graphic31* IdrawReaderImpl::load(
    const Brush* pb, const Color* pfg, const Color* pbg,
    const Font* pf, const Stipple* ps
) {
    Ref_Brush(pb);
	...
	String figname(fig->name);
		...
		y[i] = yy;
            ...
            if (figname == "Line") {
                ...
                glyph = new Open_BSpline31(brush, stroke, fill, x, y, c, tx);
            } else if (figname == "CBSpl") {
                glyph = new Closed_BSpline31(brush, stroke, fill, x, y, c, tx);
            } else if (figname == "MLine") {
                glyph = new Polyline31(brush, stroke, fill, x, y, c, tx);
            } else if (figname == "Poly") {
                glyph = new Polygon31(brush, stroke, fill, x, y, c, tx);
            } else if (figname == "Rect") {
                glyph = new Rectangle31(
		    brush, stroke, fill, x[0], y[0], x[1], y[1], tx
		);
