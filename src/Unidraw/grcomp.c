...
//        gr->SetLineHeight(font->Height());      // hack; should be state var
    ...
    } else if (cmd->IsA(TRANSFORM_CMD)) {
        Transformer* t = ((TransformCmd*) cmd)->GetTransformer();
	if (gr->GetTransformer())
	  gr->GetTransformer()->postmultiply(*t);
	else
	  gr->SetTransformer(t);
        Notify();
...
void GraphicComps::Read (istream& in) {
    ...
    for (int i = 0; i < count; ++i) {
      GraphicComp* gc = (GraphicComp*)unidraw->GetCatalog()->ReadComponent(in);
      if(gc != nil)
        Append(gc);
    }
